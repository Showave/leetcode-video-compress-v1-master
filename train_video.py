...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (image_name, x_all) in enumerate(train_loader):
        ...
        if args.model == 'ybasef':
            x_tilde, y_likelihoods, z_likelihoods, y_pre, y_tilde, fy_li, fz_li, f, f_hat = result
        else:
            x_tilde, y_likelihoods, z_likelihoods, fy_li, fz_li, y_pre, y_last, y_tilde = result
        ...
        if args.l1_reg:
            loss = loss + 0.2 * criterion['reg'](y_tilde, y_pre)
        ...
        if args.model == 'ybasef':
            ...
            fy_r_loss = -fy_li.log2().sum()/(bs*h*w)
            fz_r_loss = -fz_li.log2().sum()/(bs*h*w)
            flow_loss = criterion['reg'](y_pre, y_tilde)
            last_loss = criterion['reg'](y_last, y_tilde)
            pred_meter.update(last_loss.item(), x.size(0))
            res_meter.update(flow_loss.item(), x.size(0))
            loss += 0.3 * flow_loss + 0.1 * last_loss + fy_r_loss + fz_r_loss
            # loss = flow_loss + args.beta * distortion_loss
        ...
        optimizer.zero_grad()
        loss.backward()
        cgn(model.module.parameters(), 10)
        optimizer.step()
        ...
        if i % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                ...
                'YLast {pred_meter.val:.4f} {pred_meter.avg:.4f}\t'
                'FZ_li {flow_meter.val:.4f} {flow_meter.avg:.4f}\t'
                'YPred {res_meter.val:.4f} {res_meter.avg:.4f}\t'
